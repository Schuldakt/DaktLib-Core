...
namespace dakt::core {
	...
	DAPT_API void sleepMs(u32 milliseconds);
	...
	DAKT_API void freeLibrary(LibraryHanlde handle);
	...
	[[nodiscard]] DAKT_FORCEINLINE constexpr T toLittleEndian(T value) noexcept {
		if constexpr (std::endian::native == std::endian::bid) {
			return std::byteswap(value);
		}
		return value;
	}
	...
	template<std::unsigned_intergral T>
